...
class TestSystemFork(redistest.RedisTest):
    def setUp(self):
        ...
        self.services = {
                'orchestrator': OrchestratorClient(broker=self.broker,
                ...
                'fork': CommandLineClient(broker=self.broker,
                    exchange='fork_submit_x',
                    submit_routing_key='fork_submit_rk')}

        self.broker.register_handler(
                PetriSetTokenHandler(redis=self.conn, services=self.services,
                    queue_name='set_token_q'))
        self.broker.register_handler(
                PetriNotifyTransitionHandler(redis=self.conn, services=self.services,
    ...
    def test_system_fork(self):
        ...
        self.services['orchestrator'].set_token(net.key, 0, token_key=token.key)
        self.broker.listen()
