from flow.shell_command.lsf import request_builder
...
class LSFRequestBuilderInitTest(unittest.TestCase):
    def test_no_path_to_pre_exec(self):
        with self.assertRaises(RuntimeError):
            request_builder.LSFRequestBuilder(
                    resource_manager=None,
                    option_manager=None,
                    pre_exec=['DOESNOTEXIST'],
                    post_exec=['true'])
    ...
    def test_no_path_to_post_exec(self):
        with self.assertRaises(RuntimeError):
            request_builder.LSFRequestBuilder(
                    ...
                    option_manager=None,
                    pre_exec=['true'],
                    post_exec=['DOESNOTEXIST'])


class LSFRequstBuilderSetRequestPrePost(unittest.TestCase):
    def setUp(self):
        self.resource_manager = mock.Mock()
        self.option_manager = mock.Mock()
        self.request_builder = request_builder.LSFRequestBuilder(
                pre_exec=['true'], post_exec=['true'],
    ...
    def test_pre_exec(self):
        ...
        with mock.patch(
'flow.shell_command.lsf.request_builder.make_pre_post_command_string',
                new=make_pre_post_command_string):
            self.request_builder.set_pre_exec(request, executor_data)

        response_places = {
                'msg: execute_begin': '--execute-begin',
        }
        make_pre_post_command_string.assert_called_once_with(
                self.request_builder.pre_exec_command,
...
'flow.shell_command.lsf.request_builder.make_pre_post_command_string',
                new=make_pre_post_command_string):
            self.request_builder.set_post_exec(request, executor_data)
        ...
        make_pre_post_command_string.assert_called_once_with(
                self.request_builder.post_exec_command,
                executor_data, response_places)
...
class PrePostCommandStringTest(unittest.TestCase):
    def test_simple(self):
        ...
        self.assertEqual(expected_result,
                request_builder.make_pre_post_command_string(['EXE'],
                    executor_data, response_places))
